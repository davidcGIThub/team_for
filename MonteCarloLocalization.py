...
class MCL:
    ...
    def __init__(self, M=1000, sig_r = 0.1, sig_ph = 0.05, alpha = np.array([0.5,0.5])):
    ...
    def MCL_Localization(self, ki_past, u, z, m, dt):
        ...
        ki_bar_x = ki_past[0, :] + v_hat * dt * np.cos(ki_past[2, :]) + np.random.randn(self.M) * self.alpha[0] # propogate particles (x pos)
        ki_bar_y = ki_past[1, :] + v_hat * dt * np.sin(ki_past[2, :]) + np.random.randn(self.M) * self.alpha[0]# propogate particles (y pos)
        ki_bar_th = ki_past[2, :] + w_hat * dt + np.random.randn(self.M) * self.alpha[1] # propogate particles (theta angle)
        ...
        if np.size(m, 0) > 0:
            ...
            ki = ki_bar
            P = np.cov(ki_bar)
        mu = np.mean(ki, 1)  # estimated pose (average pose of particles)
        return ki, mu, P
